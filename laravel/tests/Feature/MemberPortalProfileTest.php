...
class MemberPortalProfileTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_portal_user_can_view_own_family_profile(): void
    {
        $response = $this->actingAs($this->portalUser, 'sanctum')
            ->getJson('/api/v1/member-portal/family');

        $response->assertStatus(200)
            ->assertJsonPath('data.family_name', 'Jane Family')
            ->assertJsonMissing(['created_by', 'approved_by']);
    }

    public function test_portal_user_can_view_own_member_profile(): void
    {
        $response = $this->actingAs($this->portalUser, 'sanctum')
            ->getJson("/api/v1/member-portal/members/{$this->member->id}");
